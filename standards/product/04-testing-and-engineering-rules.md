...
- Queue status indicators (spinner while processing, green/red status dots)
- Tooltip rendering on icon-only top bar buttons
...
- drop PDF → verify full-width rendered Markdown result in right panel (file type icon shown in queue row)
...
- queue shows spinner while processing, glossy green dot on completed, red dot on failed
...
Do not deviate from the locked panel ratios (20/20/60, CEO-approved 2026-08-07, superseding 20/35/45) without CEO approval.
...
- Simple PDF OCR works offline (with full-width rendered Markdown in right panel).
- Multi-file queue processes in FIFO order.
- Queue status indicators: spinner while processing, glossy green dot on completed, red dot on failed, thin fake progress bar.
...
- Final body font choice (CEO iterating on taste; swap-friendly via single CSS variable).
...
- Side-by-side image thumbnail compare toggle (removed from MVP 2026-08-07)
- Queue Remove button (removed from MVP 2026-08-07)
