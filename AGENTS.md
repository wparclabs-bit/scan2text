...
Queue: internal scroll, always-visible warm scrollbar; radiant rays static zero-CPU; row = icon + truncated name + size + fixed 14px dot-only status slot (grey pending / #FACC15 spinner / glossy green / glossy red) + translated tooltip + thin fake progress bar while active + retry on failed.
