...
- Frontend tests: 616 green, 0 failures. S10-FIX3 tooltip + centering fixes complete. S10-FIX4 tooltip visibility fix complete. S9.4b COMPLETE.
...
- **2026-08-14 (S10-FIX4-Tooltip-Visibility):** Root cause: FIX3 added `forceMount` to all TooltipContent (keeps all 3 mounted in DOM) + `delayDuration={200}` on TooltipProvider (200ms close-delay). Combined effect: hovering between 8px icon buttons (gap-1) keeps first tooltip open while next opens → all visible simultaneously → garbled text. Fix: removed `forceMount` from TooltipContent, removed `delayDuration` from provider. Updated tests to use synchronous `fireEvent` instead of `advanceTimersByTime`. Key lesson: test-only flags like forceMount must never ship to production UI. Tests: 616 green, 0 regressions. Status: READY FOR CEO MANUAL VERIFICATION.
